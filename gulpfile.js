...
function scripts() {
  return src([
    ...
    "app/libs/owl/dist/owl.carousel.js",

    "app/js/common.js", // Always at the endользовательские скрипты, использующие библиотеку, должны быть подключены в конце
  ])
...
function styles() {
  return src("app/" + preprocessor + "/main." + preprocessor + "") // Выбираем источник: "app/sass/main.sass" или "app/less/main.less"
    ...
    .pipe(concat("main.min.css")) // Конкатенируем в файл main.min.js
    .pipe(autoprefixer({ overrideBrowserslist: ["last 10 versions"], grid: true })) // Создадим префиксы с помощью Autoprefixer
    .pipe(
      cleancss({
        level: {
          1: { specialComments: "all", sourceMap: true },
        },
        format: "beautify",
      }),
    ) // Минифицируем стили
